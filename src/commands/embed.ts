...
export default async (
  ...
  const arg_authorIcon = options.getAttachment("authoricon") || undefined;
  ...
  const arg_footerIcon = options.getAttachment("footericon") || undefined;
  ...
  const footerIconURL = arg_footerIcon?.url;
  ...
  if (arg_timestamp === "custom") {
    if (!customTimestamp) {
      let errorEmbed = ErrorEmbedBuilder(
        ...
        "When setting `timestamp` to `'custom'` you must provide a " +
          "`customTimestamp`"
      );
    ...
    if (!isValidDate(customTimestamp)) {
      let errorEmbed = ErrorEmbedBuilder(
        ...
        "The timestamp you provided is invalid. It must be in the format " +
          "'dd/mm/YYYY HH:MM:SS'"
      );
      await interaction.reply({ embeds: [errorEmbed], ephemeral: true });
      return;
  ...
  if (timestamp) {
    embed.setTimestamp(embedTimestamp);
  }
  if (author) {
    embed.setAuthor({ name: author, iconURL: authorIconURL, url: authorURL });
  }
  if (color) {
